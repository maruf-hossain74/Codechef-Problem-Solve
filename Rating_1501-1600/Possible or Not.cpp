...
int32_t main() {
    ...
    while(t--) {
        int n, k; cin>> n>> k;
        int A[n];
        vector<int> match, AND;
        string ss = bitset<30> (k).to_string();
        for(int i = 0; i < 30; i++) {
            if(ss[i] == '1') match.push_back(i);
        }
        for(int i = 0; i < n; i++) {
            cin>> A[i];
            string s = bitset<30> (A[i]).to_string();
            bool flag = true;
            for(auto &it: match) {
                if(s[it] == '0') {
                    flag = false;
