...
int32_t main() {
    ...
    while(t--) {
        ...
        int n = s.size();
        for(int i = 1; i < n; i++) {
            if(s[i] == '1' && s[i-1] == '0') zero++;
            else if(s[i] == '0' && s[i-1] == '1') one++;
        }
        int ans = 0;
        if(one == zero) cout << n-2 << endl;
        else if(one > zero) {
            if((s[0] == '1' && s[1] == '0') || (s[0] == '1' && s[1] == '1')) ans++;
            if((s[n-2] == '1' && s[n-1] == '0') || (s[n-2] == '0' && s[n-1] == '0')) ans++;
            cout << ans << endl;
        }
        else {
            if((s[0] == '0' && s[1] == '1') || (s[0] == '0' && s[1] == '0')) ans++;
            if((s[n-2] == '0' && s[n-1] == '1') || (s[n-2] == '1' && s[n-1] == '1')) ans++;
            cout << ans << endl;
        }
